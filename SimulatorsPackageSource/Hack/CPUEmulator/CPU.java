...
public class CPU
{
    ...
    protected PointerAddressRegisterAdapter A, PC, A1, PC1;

    // The data register
    protected Register D, D1;
    ...
    // The ALU
    protected ALU alu, alu1;

    // The bus.
    protected Bus bus, bus1;
    ...
    public CPU(RAM ram, ROM rom, PointerAddressRegisterAdapter A, Register D,
               PointerAddressRegisterAdapter PC, ALU alu, Bus bus, PointerAddressRegisterAdapter A1, Register D1, PointerAddressRegisterAdapter PC1, ALU alu1, Bus bus1) {
        this.M = ram;
        this.rom = rom;
        this.A = A;
        this.D = D;
    ...
    public void executeInstruction() throws ProgramException {
        ...
        instruction = rom.getValueAt(PC1.get());
        pcChanged = false;
        if ((instruction & 0x8000) == 0)
            bus1.send(rom, PC1.get(), A1, 0);
        else if ((instruction & 0xe000) == 0xe000) {
            computeExp1(instruction);
            setDestination1(instruction);
            pcChanged = checkJump1(instruction);
        }
        else if (instruction != HackAssemblerTranslator.NOP)
            throw new ProgramException("At line " + PC1.get() +
									   ": Illegal instruction");
        if (!pcChanged) {
            short newPC = (short)(PC1.get() + 1);
            if (newPC < 0 || newPC >= Definitions.ROM_SIZE)
                throw new ProgramException("At line " + PC1.get() +
										   ": Can't continue past last line");
            PC1.setValueAt1(0, newPC, true);
        }
    ...
    // Throws ProgramException if the calculation involves M and A contains
	// an illegal address.
    protected void computeExp1(short instruction) throws ProgramException {
        boolean indirect = (instruction & 0x1000) > 0;
        boolean zd = (instruction & 0x0800) > 0;
        boolean nd = (instruction & 0x0400) > 0;
        boolean zm = (instruction & 0x0200) > 0;
        ...
        try {
            alu1.setCommand(assemblerTranslator.getExpByCode((short)(instruction & 0xffc0)),
                           zd, nd, zm, nm, f, no);
        } catch (AssemblerException ae) {}

        bus1.send(D1, 0, alu1, 0); // sends D to input0 of the alu
        ...
        if (indirect) {
			...
			if (address < 0 || address >= M.getSize())
				throw new ProgramException("At line " + PC1.get() +
										   ...
										   " is an illegal memory address.");
            A1.setUpdatePointer(true);
            bus1.send(M, address, alu1, 1);
            A1.setUpdatePointer(false);
        }
        else
            bus1.send(A1, 0, alu1, 1);
    ...
    // Throws ProgramException if destination contains M and A contains
	// an illegal address.
    protected void setDestination1(short instruction) throws ProgramException {
        boolean destA = (instruction & 0x0020) > 0;
        boolean destD = (instruction & 0x0010) > 0;
        boolean destM = (instruction & 0x0008) > 0;
        ...
        if (destA)
            bus1.send(alu1, 2, A1, 0);
        if (destD)
            bus1.send(alu1, 2, D1, 0);
    }
    ...
    // Throws ProgramException if the program counter should be changed and A
	// contains an illegal address.
    protected boolean checkJump1(short instruction) throws ProgramException {
        boolean jumpNegative = (instruction & 0x0004) > 0;
        boolean jumpEqual = (instruction & 0x0002) > 0;
        boolean jumpPositive = (instruction & 0x0001) > 0;
        boolean changed = false;

        short exp = alu1.getValueAt(2);
